(function() {
    ...
    function handleUpload(file, type) {
        if (!file) return;
        compressAvatar(file, (compressed) => {
            if (type === 'my') myAv = compressed;
            else ctAv = compressed;
            saveAllData();
        });
    }
    document.querySelectorAll('.au').forEach(b => b.onclick = () => {
        let t = b.dataset.avatar, up = t==='my'?mu:ctu;
        up.click();
        up.onchange = e => { if (e.target.files[0]) handleUpload(e.target.files[0], t); up.value = '' };
    });
    ...
    async function loadAllData() {
        ...
        if (!indexSuccess || msgs.length === 0) {
            ...
            if (chatMessages && chatMessages.msgs && chatMessages.msgs.length > 0) {
                msgs = chatMessages.msgs; nid = chatMessages.nid || 1000;
            }
        ...
        if (!window.indexedDB || !localStorage) {
            alert('检测到浏览器不支持存储，请关闭隐私模式或使用常规浏览模式。');
        }
    }
