(function() {
    ...
    async function saveMessages() {
        ...
        const data = { id: 'main', msgs: msgs.map(m => {
            const clean = {...m};
            if (clean.objectUrl) delete clean.objectUrl;
            return clean;
        }), nid };
        store.put(data);
    ...
    async function loadMessages() {
        ...
        return new Promise((res, rej) => {
            ...
            req.onsuccess = () => {
                if (req.result) {
                    msgs = req.result.msgs || [];
                    nid = req.result.nid || 1000;
                }
                res();
            };
            req.onerror = () => rej(req.error);
    ...
    async function loadLetters() {
        ...
        return new Promise((res, rej) => {
            const req = store.get('main');
            req.onsuccess = () => {
                if (req.result) letters = req.result.letters || [];
                res();
            };
            req.onerror = () => rej(req.error);
        });
    ...
    async function initIDB() {
        ...
        // 为图片消息创建 Object URL
        for (const m of msgs) {
            if (m.msgType === 'image' && m.imageId != null) {
                const blob = await getImageBlob(m.imageId);
                if (blob) {
    ...
    function safeStorageSet(key, data) {
        try { localStorage.setItem(key, JSON.stringify(data)); } catch(e) { alert('存储空间已满，请清理一些数据后再试。'); }
    }

    async function saveAllData() {
        safeStorageSet('chatSettings', { myName, ctName, myAv, ctAv, rDelay, aMin, atChecked: at.checked, etChecked: et.checked });
        safeStorageSet('wordCards', { textCards, emojiCards, imageCards, statusCards, groups });
        safeStorageSet('chatStickers', chatStickers);
        await saveMessages();
        await saveLetters();
    }
    ...
    function updateContactStatus() {
        if (statusCards.length > 0) {
            contactStatusEl.textContent = statusCards[Math.floor(Math.random()*statusCards.length)].content;
        } else { contactStatusEl.textContent = '在线'; }
        clearTimeout(statusTimer);
        statusTimer = setTimeout(updateContactStatus, Math.floor(Math.random() * 8 * 3600000) + 3600000);
    }
    ...
    function sendRapidReplies() {
        ...
        function sendNext() {
            ...
            showTyping();
            rapidReplyTimer = setTimeout(async () => {
                hideTyping();
                const msgObj = await getRandomReplyMessage();
                if (msgObj) {
                    const msg = { id: nid++, senderId: CT, timestamp: Date.now(), status: 'read' };
                    if (msgObj.type === 'image') {
                        msg.msgType = 'image';
                        msg.imageId = msgObj.imageId;
                        msg.text = '';
                    } else {
                        msg.text = msgObj.content;
                    }
    ...
    async function simReply() {
        ...
        rTimer = setTimeout(async () => {
            ...
            if (!msgObj) return;
            const msg = { id: nid++, senderId: CT, timestamp: Date.now(), status:'read' };
            if (msgObj.type === 'image') { msg.msgType = 'image'; msg.imageId = msgObj.imageId; msg.text = ''; }
            else { msg.text = msgObj.content; }
            msgs.push(msg); render(); saveAllData();
        }, rDelay);
    }
    ...
    async function render() {
        ...
        let h = '';
        for (const m of msgs) {
            if (m.senderId === SYS) { h += `<div class="mr msg-system"><span>${m.text}</span></div>`; continue; }
            ...
            let bubbleContent;
            if (m.msgType === 'image') {
                const imgSrc = m.objectUrl || (m.imageId ? URL.createObjectURL(await getImageBlob(m.imageId)) : '');
                bubbleContent = `<div class="mb img-bubble"><img src="${imgSrc}" alt="图片"></div>`;
            } else {
    ...
    function send(txt, q = null, msgType = 'text', imageBlob = null) {
        if (isSending) return;
        isSending = true;
        const msg = { id:nid++, senderId:MY, timestamp:Date.now(), status:'unread', quoteId:q?.id, quoteText:q?.msgType==='image'?'[图片]':q?.text };
        if (msgType === 'image' && imageBlob) {
            (async () => {
                const imageId = await storeImageBlob(imageBlob);
                msg.msgType = 'image';
                msg.imageId = imageId;
                msg.text = '';
                msgs.push(msg);
    ...
    chatImageInput.onchange = e => {
        ...
        Array.from(files).forEach(f => {
            compressImage(f, blob => { send('', null, 'image', blob); });
        });
    ...
    function renderStickerPanel() {
        ...
        chatStickers.forEach((stk, idx) => {
            ...
            item.querySelector('img').addEventListener('click', () => {
                const blob = base64ToBlob(stk);
                send('', null, 'image', blob);
                stickerPanel.classList.remove('show');
            });
            const delBtn = item.querySelector('.sticker-del');
            delBtn.addEventListener('click', (e) => { e.stopPropagation(); chatStickers.splice(idx,1); saveAllData(); renderStickerPanel(); });
            delBtn.addEventListener('touchend', (e) => { e.stopPropagation(); chatStickers.splice(idx,1); saveAllData(); renderStickerPanel(); });
    ...
    stickerFileInput.onchange = e => {
        ...
        if (!files.length) return;
        Array.from(files).forEach(f => {
            compressImage(f, blob => {
                const reader = new FileReader();
                reader.onload = ev => { chatStickers.push(ev.target.result); saveAllData(); renderStickerPanel(); };
    ...
    function startDialing() {
        ...
        dialTimer = setTimeout(() => { if (Math.random() < 0.6) { isDialing = false; startCall(true); } else { endCall(); } }, delay);
    }
    ...
    function scheduleIncomingCall() {
        ...
        const delay = Math.floor(Math.random() * (110 * 60 * 1000)) + (10 * 60 * 1000);
        incomingCallTimer = setTimeout(() => {
            if (!inCall && !incomingWaiting && !isDialing) { incomingCall(); }
        ...
        reader.onload = async (ev) => {
            try {
                ...
                if (Array.isArray(data)) {
                    if (confirm('导入将替换当前聊天记录，确定继续吗？')) { msgs = data; nid = Math.max(...msgs.map(m => m.id),0)+1; render(); renderHist(); saveAllData(); }
                } else if (data && Array.isArray(data.msgs)) {
                    if (confirm('导入将替换当前聊天记录，确定继续吗？')) { msgs = data.msgs; nid = Math.max(...msgs.map(m => m.id),0)+1; render(); renderHist(); saveAllData(); }
    ...
    function renderGroupBar() {
        ...
        let h = groups.map(g => {
            return `<div class="group-tag ${currentGroupFilter===g.id?'active':''}" data-group-id="${g.id}" style="border-color:${g.color}"><span class="group-color" style="background:${g.color}"></span>${g.name}<button class="group-edit-btn" data-edit-group="${g.id}">✎</button>${g.id!=='default'?`<button class="group-edit-btn" data-del-group="${g.id}">✕</button>`:''}</div>`;
        }).join('');
    ...
    function importText(text, type, groupId='default') {
        ...
        let arr = getCardArrByType(type);
        let added = 0;
        lines.forEach(l => { if (!arr.some(c => c.content === l)) { arr.push({ id: Date.now()+Math.random(), content: l, groupId: (type==='text'||type==='status')?groupId:undefined }); added++; } });
        if (added > 0) { renderWB(); saveAllData(); }
        alert(`导入了 ${added} 条`);
    }
    wbImportText.onclick = () => { if (currentTab === 'image') { imgUploadInput.click() } else { importArea.style.display = 'block'; importTextArea.value = ''; } };
    wbUploadImg.onclick = () => imgUploadInput.click();

    confirmImport.onclick = () => { let gid = currentTab==='text'? (currentGroupFilter!=='all'?currentGroupFilter:'default') : 'default'; importText(importTextArea.value, currentTab, gid); importTextArea.value = ''; importArea.style.display = 'none'; };
    ...
    imgUploadInput.onchange = e => {
        ...
        Array.from(files).forEach(f => {
            compressImage(f, blob => {
                const reader = new FileReader();
                reader.onload = ev => { addCard(ev.target.result, 'image'); };
                reader.readAsDataURL(blob);
            });
        });
